...
//set the constructor from object to enemy
...
Enemy.prototype.update = function (dt) {
    ...
    for (let i = 0; i < allEnemies.length; i++) {
        ...
        //Changes the speed of the bugs depending on your score
        if (player.score === 0 || player.score <= 2000) {
            allEnemies[i].x += (low * dt);
        } else if (player.score >= 1001 || player.score <= 5000) {
...
Player.prototype.update = function () {
    //Scoreboard update
    ScoreBoard.innerHTML = `Score: ${player.score} Lives: ${player.lives}`;
...
//Checks bugs postion to see if it matches player
...
function checkCollision(x, y) {
    ...
    allEnemies.forEach(ene => {
        if ((ene.x + pad >= coords.xBegin - pad && ene.x + pad <= coords.xEnd - pad) && (ene.y >= coords.yBegin && ene.y <= coords.yEnd)) {
            //collision has occured, resets your position back to the start and removes a life if you have one.
            player.y = 380;
            player.lives -= 1;
            //if you have zero lives the game will end and a scoreboard will popup with your score.
            if (player.lives === 0) {
                alert(`Game Over!\nHere is your score ${player.score}\nOnce you click Ok. Your game will restart! `);
                player.score = 0;
                location.reload()
